...
void euclid_lsh::set_row(const string& id, const common::sfv_t& sfv) {
  ...
  get_table()->add(id, owner(my_id_), cosine_lsh(sfv, hash_num_, threads_, cache_), l2norm(sfv));
}
...
void euclid_lsh::set_config(const config& conf) {
  ...
  jubatus::core::nearest_neighbor::init_cache_from_config(cache_, conf.cache_size);
}
...
void euclid_lsh::neighbor_row_from_hash(
  ...
  jubatus::util::lang::function<heap_t(size_t,size_t)> f =
    jubatus::util::lang::bind(&ranking_hamming_bit_vectors_worker, &bv, &bv_col, &norm_col,
                              denom, norm, ret_num,
