...
public:
  random_projection_cache(int size) : lru(size), lock() {}
...
std::vector<float> random_projection(const common::sfv_t& sfv, uint32_t hash_num, uint32_t threads, cache_t& cache);
