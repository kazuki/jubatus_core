...
static std::vector<float> random_projection_internal(
  const jubatus::core::common::sfv_t& sfv, uint32_t hash_num, size_t start, size_t end, cache_t& cache)
...
// returning true if cache hitted
inline static void init_cache(uint32_t hash_num, vector<float>& proj, const cache_t& cache);
inline static bool check_cache(cache_t& cache, uint32_t seed, vector<float>& proj, float v);
template<int N>
inline static void build_cache_if_enabled(const cache_t& cache, vector<float>& grnd_cache, const float *grnd);
inline static void lru_set_cache_if_enabled(cache_t& cache, uint32_t seed, vector<float>& grnd_cache);
...
bit_vector cosine_lsh(const common::sfv_t& sfv, uint32_t hash_num, uint32_t threads, cache_t& cache) {
  return binarize(random_projection(sfv, hash_num, threads, cache));
}

vector<float> random_projection(const common::sfv_t& sfv, uint32_t hash_num, uint32_t threads, cache_t& cache) {
